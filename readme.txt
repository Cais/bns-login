...
= Can I use this in a widgetized area? =
Yes, but not in the traditional drag-and-drop method. Simply use the `bns_login` shortcode (in square brackets, no quotes) in a standard Text Widget.
As a side benefit, this plugin will also allow other shortcodes to be used in text widgets if that functionality was not previously available.
See this WordPress codex entry for more information on shortcodes: http://codex.wordpress.org/Shortcode
